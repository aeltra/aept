...
# make_aep_hardlink <out.aep> <name> <link-target>
#
...
root=$work/root-ok
new_root "$root"
make_aep_hardlink "$work/inside.aep" inside usr/bin/inside

out=$(aept_run "$root" install --non-interactive "$work/inside.aep" 2>&1)
rc=$?
...
new_root "$root"
make_aep_hardlink "$work/escape.aep" escape ../../../../../../etc/passwd

out=$(aept_run "$root" install --non-interactive "$work/escape.aep" 2>&1)
rc=$?
