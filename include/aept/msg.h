...
enum aept_log_level { AEPT_ERROR, AEPT_WARNING, AEPT_INFO, AEPT_DEBUG };
...
#define aept_log_error(fmt, ...) aept_log(AEPT_ERROR, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define aept_log_warning(fmt, ...) aept_log(AEPT_WARNING, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define aept_log_info(fmt, ...) aept_log(AEPT_INFO, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define aept_log_debug(fmt, ...) aept_log(AEPT_DEBUG, __FILE__, __LINE__, fmt, ##__VA_ARGS__)

int aept_confirm_continue(void);
void aept_display_transaction(const struct aept_transaction *txn);
void aept_print_heading(const char *fmt, ...) __attribute__((format(printf, 1, 2)));
