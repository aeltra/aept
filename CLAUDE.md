...
- **archive.c** — Two-level extraction (outer AR → inner tar), the `.deb`/`.ipk` container layout. Handles nested decompression with libarchive callbacks. Originally adapted from opkg and GPL-licensed; **rewritten from scratch and relicensed MIT in `4f0989d`** — do not reintroduce opkg code here. Compression support (gzip always; xz/bzip2/lz4/zstd compile-time via `HAVE_*`). An `ARCHIVE_WARN` from `archive_read_extract2()` is accepted only if the entry is actually on disk afterwards: libarchive uses that code both for "could not restore the mtime" and for an entry it could not create at all — a file below a path that is a regular file, which `ARCHIVE_EXTRACT_NO_OVERWRITE` leaves in place without a word — and taken as success the second one produced an install whose `.list` named a file that was never written.
