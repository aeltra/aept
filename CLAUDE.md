...
One *inserted* or *renumbered* is a break — and so would be one *removed*
from the middle, since everything after it renumbers. Renaming one in place
is neither: every value stays, and a caller using the old name has a
source-level change to make, not a binary one.
...
- Shell tests source `tests/aeptlib.sh` (`make_pkg`, `make_pkg_conffile`,
  `make_pkg_script`, `make_pkg_tree`, `make_pkg_scripts`, `packages_stanza`, `add_repo`,
...
- **Every relationship field has a contract test**, because a resolver
  ...
  clash.c. **`tests/test_script_order.sh` is the maintainer-script
  contract**: every package in it ships all four scripts, each logging
  its name and arguments, and each section asserts the *whole*
  sequence for one path — install, upgrade, supersede, disappear, and
  every failure point's unwind. Add a section there for any change to
  when a script runs; assert the sequence, not the one call.
  `tests/test_integrity.sh` does the same for `aept verify`: one kind of
  damage at a time, the whole reported line compared. A fixture subtlety: a package's relationships once
...
**Persistence is assumed, not waited for.** The other half of "goes back in the cache" is whether the connection is reusable at all, and libfetch inherited that backwards: `keep_alive` started at 0 and was turned on only by a `Connection: keep-alive` in the response. That is HTTP/1.0's rule. HTTP/1.1 is persistent by default (RFC 9112 §9.3) and a server says so by staying silent — nginx and Caddy send no `Connection` header at all — so against `archive.aeltra.eu` the cache never engaged **once**, and every index and every package paid a fresh `getaddrinfo()` and a fresh handshake. Measured over five downloads of one URL: 10 `connect()` calls (five of them the unreachable AAAA) and 5 DNS queries before, 2 and 1 after. The default now comes from the reply's version, which `http_get_reply()` records in `conn->http_minor` rather than merely validating, and a `Connection` header overrides it in either direction. The field is a **token list** — `keep-alive, TE` and `TE, close` are both legal — so `http_has_token()` parses it instead of comparing the whole value against one token, and `close` wins when a peer names both. The six cases in `test_http.sh`'s reuse section pin the whole decision; two of them (`/implicit-ka`, `/ka-list`) are the ones that were red, and the other four exist so the fix cannot overshoot into "always persistent" — `/close-list` and `/http10` have the stub hold the connection open on purpose, since a server that hangs up produces one connection per request whatever the client believed, and the count would prove nothing.

A knock-on worth knowing: the per-download `getaddrinfo()` was also a per-download IPv6 connect attempt on any host whose AAAA is unreachable. That is not fixed — `libfetch_connect()` still walks the address list with `option network_timeout` as its per-address bound, so a blackholed AAAA still costs the full timeout — but it is now paid once per run instead of once per file.
...
- **archive.c** — Two-level extraction (outer AR → inner tar), the `.deb` container layout. Handles nested decompression with libarchive callbacks. Originally adapted from opkg and GPL-licensed; **rewritten from scratch and relicensed MIT in `4f0989d`** — do not reintroduce opkg code here. Compression support (gzip always; xz/bzip2/lz4/zstd compile-time via `HAVE_*`).

  **Every file is written aside and renamed into place.** A regular file, symlink or hard link is extracted as `<path>.aept-new` and `rename()`d over its path, so the path holds the old object or the whole new one and never part of either — a write that fails half-way, for want of space or by a kill, leaves the old file where it was and the partial copy is removed (`tests/test_atomic_unpack.sh` proves it with a tmpfs that runs out). That is dpkg's per-file atomicity; not yet its per-package unwind, which would keep the displaced originals as `.aept-tmp` until the package is done and restore them on failure. A conffile stays at its aside name for conffile.c to decide. Directories are made in place. **Regular-file data is written by `write_regular()`, not `archive_read_extract2()`**: libarchive reports a `write()` that fails — `ENOSPC`, `EIO` — as `ARCHIVE_WARN`, the same code as "could not restore the mtime", and then `ftruncate()`s the file to the entry's full size, so a package that did not fit came out as a file of the right length with a hole where the data should be, and no error. Directories, symlinks and hard links have no data and stay with libarchive, with an `lstat()` after any `ARCHIVE_WARN` because the same code also covers an object it could not create. **libarchive's own knobs do not help here, and this was measured before deciding**: with `ARCHIVE_EXTRACT_SAFE_WRITES` (its aside-and-rename mode) and the disk writer driven directly — `archive_write_header()` / `archive_write_data()` / `archive_write_finish_entry()`, no `archive_read_extract2()` in the way — an `ENOSPC` on libarchive 3.7.4 came back as `ARCHIVE_WARN` (-20) from the data call, `ARCHIVE_OK` from the finish call with the error string cleared, and the temporary was renamed over the original regardless: a 400 KB file of the right length on a 128 KB filesystem, old content gone. The disk writer's error model is bsdtar's "warn and carry on", and no flag changes it.

  Two things libarchive did on its own that aept now refuses, each with a unit case in `tests/test_archive_open.c`: `rmdir` an empty directory to put a file in its place (the rename refuses that by itself), and keep a regular file found where a directory entry lands, under `ARCHIVE_EXTRACT_NO_OVERWRITE`, and report success (the directory is checked to be one afterwards). A third is kept deliberately: a missing parent directory is made on the way, as libarchive made it. A package ships an entry for every directory it writes into, and one made this way is in nobody's `.list` and outlives its package — but refusing the package for it would be the larger harm. Device nodes, FIFOs and sockets are refused at listing time, where the clash check reads the package before any script has run, and again at extraction; a hard link's header carries no file type of its own and must not be mistaken for one.
- **install.c** — Orchestrates: load repos → solve → download → then, per step, **unpack** (`do_install_package()` / `do_upgrade_package()`) and **configure** (`configure_package()`), with `remove_superseded()` between them. The maintainer-script sequence is dpkg's, and `tests/test_script_order.sh` asserts every line of it by having each script log its name and arguments:

  | | |
  |---|---|
  | install | clash check → `preinst install` → unpack → *(disappear, supersede)* → recorded `unpacked` → `postinst configure` → `installed` |
  | upgrade | clash check → `old-prerm upgrade new` → `new-preinst upgrade old new` → unpack → `old-postrm upgrade new` → delete files the new version dropped → replace scripts and list → *(disappear, supersede)* → `unpacked` → `new-postinst configure old` → `installed` |
  | remove | `prerm remove` → delete files → `postrm remove` → clean info dir |

  Three things in that table were each got wrong once. **The clash check runs before any script**: it needs only the archive listing and the owner index, and a refusal is ordinary, so it must not come after a prerm that has already stopped a daemon. **The old postrm runs before the old files are deleted**, not after: it may need them. And **the failed step is unwound** — `preinst` or unpack failing runs `new-postrm abort-install` (or `abort-upgrade old new` followed by `old-postinst abort-upgrade new`, which is what restarts what the old prerm stopped); an old `prerm upgrade` failing gives the new version's `prerm failed-upgrade old new` a chance, and only if that fails too does the upgrade abort with `old-postinst abort-upgrade`; an old `postrm upgrade` failing tries `new-postrm failed-upgrade old new` and carries on either way; a `prerm remove` failing runs `postinst abort-remove`. There are no file backups, so an unwind after a failed unpack cannot put the old files back — the old postinst is still the only script that can restart the service.

  **Unpack and configure are separate steps** so that what a package supersedes (Conflicts + Replaces) can be removed in between: the old package must still be there when the new one is unpacked, so its files are taken over rather than vacated and refilled, and gone before the new one is configured, whose postinst would otherwise start a daemon on a port the old one still holds. That is the order dpkg keeps by removing a conflictor inside the unpack of the package replacing it. `remove_superseded()` pulls those ERASE steps forward and marks them in `done[]`; solver.c's `order_takeovers()` still guarantees they come *after* the install in the step list, which is what the clash check relies on. If the superseded package will not go — its prerm fails — the new one stays `unpacked` and is not configured, since configuring it beside the package it conflicts with is what the order exists to prevent. Everything else is still one step per package with the dependency fully configured before its dependant is unpacked, which is stricter than Policy asks and makes `Pre-Depends` trivially satisfied; a dependency cycle is the one place a postinst can run before its dependency is even unpacked.
- **remove.c** — Orchestrates: solve removal → prerm → delete files from .list → postrm → clean info dir → update status. There is no purge: `--purge` extends the deletion to modified conffiles but the postrm is still called with `remove`, and the postrm goes with the other info files, so a package removed without `--purge` cannot be purged later.
- **status.c** — Reads/writes the installed-packages database (Debian control format). Loaded into libsolv as the "@installed" repo.
- **update.c** — Fetches package lists. Carries on past a source that fails and returns `-1` at the end, so what it reports is an **aggregate**, and it sets `AEPT_ERR_GENERAL` regardless of what any one source hit. A per-source `AEPT_ERR_TIMEOUT` must not be passed up: it says "momentary, just now, try again", and may have happened a successful source ago — and which failure came *last* depends on the order the sources are configured in, so the same failures would report differently from one config to the next. The rule that falls out: **`AEPT_ERR_TIMEOUT` is reported only by operations that stop at it** — install, upgrade and remove abort at the first failed download, so there it always means "this is why we stopped". `tests/test_update_error.sh` drives `tests/updateerr.c` (the CLI folds the classification into an exit status, so a harness prints it) against a stalled source and a good one in both orders. With `check_signature` on (the default) it fetches **`InPackages.gz`** — index and signature in one object — and there is deliberately **no** fallback to `Packages` + `Packages.sig`, so blocking one request cannot push a client onto the two-object path. Decompression is capped at `MAX_INDEX_SIZE`, since the stream is chosen by the remote side. **The signature is checked here and nowhere else** — `aept_verify_signature()` has exactly one caller — so the stored index is trusted thereafter because it was verified once, when written. The `.sig` written beside it is never read again. The new index is written *aside*, verified there, and renamed into place only on success: verifying in place cost the client its last good index on every rejected update, which turned "freeze the client" — the strongest attack a bad signature affords — into "break the client". A refusal leaves the old index, its `.sig` and its validator exactly as they were; `tests/test_signature_reject.sh` holds this down.
...
- **sums.c** — a package's shipped `sha256sums` (the control-archive member the Aeltra build tool writes: `<hex>  <path>`, `sha256sum(1)`'s format, regular files only, a hard link's second name with its own line). Read whole and refused whole: a list that is there and wrong — a bad line, a duplicate, an empty file — refuses the *package*, since a digest list that cannot be read is not one to trust half of. Sorted for `bsearch`; leading `./` and `/` ignored on both sides. **What it buys is a check at the only point in the chain that reaches the file itself**: signed index → package sha256 → gzip CRC → nothing, until now. `write_regular()` hashes every regular file as the bytes go by (OpenSSL `EVP`, not libsolv's `solv_chksum` that download.c uses for package checksums: measured on an x86_64 without SHA extensions, libsolv's portable C does 210 MB/s and OpenSSL 453, and with SHA-NI or ARMv8 crypto OpenSSL is several times faster again — hashing 100 MiB of files cost 0.50 s on top of a 0.12 s install with libsolv and 0.25 s with OpenSSL, so this is the one place in the tree where the digest library is the install's dominant cost; the holes of a sparse entry are fed to it as zeros, since they are zeros to a reader), and with a shipped list the digest is compared **before the rename** — a file that does not match never goes into place, an archive file the list lacks and a listed file the archive lacks both refuse the package, and the counts are checked at the end so the two sides must agree exactly. A package without the member (every one in the current archive) records aept's own digest. Both are `tests/test_sha256sums.sh`; the unit cases are in `test_archive_open.c`, including the one where the counts balance so only the per-file check can catch an unlisted file, and the sparse one. What is recorded beside the digest is **what is on disk after extraction** — owner and group from `fstat()` after the `fchown()`, so an `ignore_ownership` root does not verify every file as wrong-owner; a hard link's mode from `lstat()`, its header carrying none. **Ownership needs telling when it may fail.** `option ignore_ownership 1` (`AEPT_FLAG_IGNORE_OWNERSHIP`) says a failed `chown` is expected: aept still *tries* — so inside a user namespace the ids that are mapped, like 0, do get set — and a file it could not give to its owner stays whoever's it is, minus its setuid and setgid bits, since they would grant an identity the package did not mean. Without the option a failed `chown` is an error, and the message names the option. Two environments need it, both build-box's: an unprivileged install (the target root is assembled as a user, for compiling in and not for running), and **root inside a user namespace**, where aept is uid 0, `chown` to a mapped id works, and `chown` to an id the namespace does not map (`www-data`'s 33) fails with `EINVAL` however root it looks — `test_atomic_unpack.sh` found that one, since it runs aept under `unshare -Urm`. A uid-map sniffing variant that decided this without configuration was tried and rejected: whether a failed `chown` is acceptable is a property of the root, not of the process. `tests/test_unprivileged_owner.sh` has both environments, with and without the option, skipped as root. Two consequences for fixtures: **every test package's tar carries `--owner=0 --group=0`**, as real packages do — a fixture owned by the test user is unmapped inside a namespace and refuses to install there — and `new_root` sets the option, since the suite runs as a user. One thing to keep: the entry's name is copied once (`entry_name`) because `rewrite_all_paths()` and the aside rename replace libarchive's pathname, and a pointer into it named garbage in every message after that point — it did, in the device refusal, until the sums lookup made it visible.
- **integrity.c** — `aept verify`: every line of a package's `.list` held against the object at that path, one `aept_verify_entry_t` per difference. What is *recorded* decides what is *checked* — a column an older list lacks is not held against anything, and a regular file with no digest at all is reported `unverifiable` rather than passed, so a root from before digests were recorded is not silently "clean". A directory with a filesystem mounted on it (its `st_dev` differs from its parent's) is skipped whole — on a live root `/proc` and `/sys` are kernel-owned 0555 whatever `base-files` shipped, and the first run against a real build-box target reported exactly those two; note the `.list` records directories with a trailing slash, so the parent lookup trims it first. The kinds are ordered from "nothing there" inward: `missing`, `type` (and nothing further — comparing a directory's size to a file's is noise), `mode`, `owner` (only without `ignore_ownership`, since ownership was never applied there; `--ignore-ownership` sets the same flag for one call), `link`, then for a regular file `size` *or* `digest`, not both. A conffile is compared against the md5 the conffile record keeps and reported as `conffile`, which is not damage: the admin's edit is the expected state. The CLI exits 0 for nothing damaged, 1 for damage, 2 for an error — `conffile` and `unverifiable` are reported and do not fail, which is what lets `aept verify` gate a health check on a real device. Repair is `aept install --reinstall`, and the test proves it restores every case but the conffile, which a reinstall rightly leaves alone. Digests are OpenSSL `EVP`, like `write_regular()`'s; whole-file reads, so `verify` of a root costs one pass over its files. `tests/test_integrity.sh` produces each kind of damage one at a time and compares the *whole* output line and the exit status — those lines are the contract a script would parse.
