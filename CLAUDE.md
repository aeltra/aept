...
is neither: `AEPT_FLAG_IGNORE_UID` became `AEPT_FLAG_IGNORE_OWNERSHIP` in
the same slot and every value stayed — a source-level change for a caller
using the old name, not a binary one.
