<!-- Generated from aept.1.scd by scripts/render-manual.sh.
     Do not edit: run "make docs" instead. -->
...
## verify \[options\] \[packages...\]

Check the files of the named packages -- of every installed package when
none is named -- against what was recorded when they were installed:
presence, type, permission bits, owner and group, a symlink's target,
and a regular file's size and SHA-256. One line per difference, naming
the kind, the path, the package and, where it applies, what was recorded
and what was found:

    digest       /usr/bin/foo  (foo)  5891b5…  -> c9c354…
    missing      /usr/lib/libbar.so.1  (libbar)  was regular file

Two kinds are reported without being counted as damage: *conffile*, a
configuration file the administrator has changed, compared against the
hash the conffile record keeps; and *unverifiable*, a regular file whose
record carries no digest because it was installed before digests were
recorded. Owner and group are not checked when **ignore_ownership** is
set.

Exit status is 0 when nothing is damaged, 1 when something is, and 2 on
an error, including a named package that is not installed.

**--ignore-ownership**

> Do not check owner and group for this run.
...
| ignore_ownership   | 0                             | Set to 1 when the root is not installed as root: a file that cannot be given to the owner the package names is kept as it is, minus its setuid and setgid bits, instead of failing the install.                                                                                      |
...
| install_recommends | 0                             | Set to 1 to install a package's **Recommends** along with it, as **apt**(8) does. Off by default: a recommendation is not a requirement, and following it pulls in packages nobody named -- transitively -- which is rarely what a small root wants. **Suggests** is never followed. |
